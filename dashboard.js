...
// Investment plan links (UPDATED WITH YOUR SELAR LINKS)
const investmentLinks = {
    starter: "https://selar.com/0v1thj",
    silver: "https://selar.com/1447x0",
...
// Investment plan selection - redirects to external site (OLD, NOT NEEDED IF USING investBtns)
...
// Invest now buttons - redirect to Selar payment links (UPDATED)
investBtns.forEach(btn => {
    btn.addEventListener('click', function() {
        const paymentLink = this.getAttribute('data-link'); // Get link from HTML
        if (paymentLink) {
            window.location.href = paymentLink; // Redirect to Selar
        }
    });
});
